...
CGeneralEntity Aimbot::GetTarget(CGeneralEntity e) {
	...
	for (int i = 0; i <= 128; i++) { // Loop through all entities
		CGeneralEntity temp(Mem->Read<DWORD>(pLocalPlayer->ClientDLL + offset::dwEntityList + (i * 0x10))); // Temporary entity in loop
		...
		if (temp.ClassID != 35)
			continue;
		/*if (!VisibleCheck->IsVisible()) // SPOTTED CHECK
			continue;*/
...
void Aimbot::SetAngle(D3DXVECTOR3 ViewAngle) {
	WriteProcessMemory(Mem->hProcess, (LPVOID)(pLocalPlayer->getEnginePointer() + offset::dwClientState_ViewAngles), &ViewAngle, sizeof(D3DXVECTOR3), NULL);
}
